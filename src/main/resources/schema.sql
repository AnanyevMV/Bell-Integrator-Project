...
CREATE TABLE organization (
  ...
  CONSTRAINT CHK_organization_is_active CHECK (
    (is_active = 0)
    OR (is_active = 1)
  ),
...
CREATE TABLE office (
  ...
  CONSTRAINT CHK_office_is_active CHECK (
    (is_active = 0)
    OR (is_active = 1)
  ),
...
CREATE TABLE document_type (
  doc_code VARCHAR(255) COMMENT 'Код документа' PRIMARY KEY,
  doc_name VARCHAR(255) COMMENT 'Название документа' NOT NULL,

  UNIQUE INDEX UX_document_type_name (doc_name)
);

DROP TABLE IF EXISTS document;
CREATE TABLE document (
  doc_code VARCHAR(255) NOT NULL COMMENT 'Код документа',
  doc_number VARCHAR(255) NOT NULL COMMENT 'Номер документа',
  doc_date DATE COMMENT 'Дата' NOT NULL,
...
CREATE TABLE user (
  ...
  CONSTRAINT CHK_user_is_identified CHECK (
    (is_identified = 0)
    OR (is_identified = 1)
  ),
  ...
  ON DELETE NO ACTION ON UPDATE NO ACTION,
  INDEX IX_user_doc_code_doc_number (doc_code, doc_number),
